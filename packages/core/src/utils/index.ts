...
function stringifyYAML(obj: unknown): string {
  ...
  const lines: string[] = [];
  function stringifyValue(val: unknown, indent: number = 0): string {
    const spaces = " ".repeat(indent);
    if (val === null || val === undefined) return `${spaces}null`;
    if (typeof val === "string") return `${spaces}"${val}"`;
    if (typeof val === "number" || typeof val === "boolean")
      return `${spaces}${val}`;
    if (Array.isArray(val)) {
      if (val.length === 0) return `${spaces}[]`;
      return val
