...
// Helper Functions
// ============================================================================
async function selectAgent(message = 'Select an agent:') {
 ...
 * but clearly marked.
 */
async function selectTargetAgents(message = 'Select target agent(s):') {
...
providerCmd
    ...
    if (!await confirmAction(`Remove provider "${providerId}"?`))
        return;
...
modelCmd
    ...
    if (!await confirmAction(`Remove model "${modelId}"?`))
        return;
...
mcpCmd
    ...
    server.approvalMode = approvalMode;
    const spinner = ora('Adding MCP server...').start();
    const result = await manager.addMCPServer(agentId, server);
    spinner.stop();
    if (result.success) {
...
mcpCmd
    ...
    if (!await confirmAction(`Remove MCP server "${serverName}"?`))
        return;
...
permCmd
    ...
    if (!await confirmAction(`Remove permission "${permissionId}"?`))
        return;
...
program
    ...
    if (options.mcp) {
        ...
        const server = { name, type: type, enabled: true };
        if (type === 'stdio') {
        ...
        const result = await manager.addMCPServerToAll(server);
        spinner.stop();
        if (result.success) {
...
program
    ...
    if (!await confirmAction(`Restore config from "${backupPath}"?`))
        return;
...
program
    ...
    const path = manager.getConfigPath(agentId, options.platform);
    if (path) {
        console.log(path);
    }
    else {
