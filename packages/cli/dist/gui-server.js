...
import { probeProviderAPIs, getAgentCatalog, getAgentCatalogEntry, getAgentCatalogMeta, getAgentCommands, isSafeCommand, detectCatalogEntry, catalogEntryToDetected, } from '@ai-agent-config/core';
// ============================================================================
// Port selection
// ============================================================================
/**
...
function randomPort() {
    ...
    for (let i = 0; i < 64; i++) {
        ...
        if (!EXCLUDED_PORTS.has(candidate))
            return candidate;
    }
...
function resolveDistDir(override) {
    if (override)
        return path.resolve(override);
    if (process.env.AI_CONFIG_DIST)
        return path.resolve(process.env.AI_CONFIG_DIST);
...
function startAgentJob(agentId, action, command, options) {
...
export async function startGuiServer(manager, options = {}) {
    ...
    const server = http.createServer(async (req, res) => {
        ...
        const wrap = async (data, status = 200) => handle(async () => ({ status, data }));
        try {
            // ---- GET /api/state ----
            ...
            if (parts[1] === 'providers') {
                ...
                if (method === 'POST' && parts.length === 3 && parts[2] === 'verify') {
                    ...
                    return handle(async () => {
                        if (!body.baseUrl)
                            return { error: 'baseUrl is required', status: 400 };
                ...
                if (method === 'POST' && parts.length === 4 && parts[3] === 'test') {
                    ...
                    return handle(async () => {
                        ...
                        if (!entry)
                            return { error: `Provider "${parts[2]}" not found in registry`, status: 404 };
                        const config = (entry.provider.config || {});
                        const baseUrl = String(config.baseUrl || '');
                        if (!baseUrl)
                            return { error: 'This provider has no base URL configured — add one first', status: 400 };
                        const result = await probeProviderAPIs({
                            baseUrl,
                        ...
                        if (!saved.success)
                            return { error: saved.error, status: 500 };
                        return { data: result };
                    });
                }
                if (method === 'POST' && parts.length === 2) {
                    const body = await readBody();
                    return handle(async () => {
                        const result = await manager.registerProvider(body.provider, body.models || [], body.agentIds || [], body.apiCapabilities);
                        if (!result.success)
                            return { error: result.error, status: 400 };
                        return { data: { registry: result.data, warnings: result.warnings } };
                    });
                ...
                if (method === 'PUT' && parts.length === 3) {
                    ...
                    return handle(async () => {
                        const result = await manager.updateProvider(parts[2], {
                            ...
                            models: body.models,
                            apiCapabilities: body.apiCapabilities,
                        });
                        if (!result.success)
                            return { error: result.error, status: 400 };
                ...
                if (method === 'POST' && parts.length === 4 && parts[3] === 'agents') {
                    const body = await readBody();
                    return handle(async () => {
                        const result = await manager.addProviderToAgents(parts[2], body.agentIds || []);
                        if (!result.success)
                            return { error: result.error, status: 400 };
                        return { data: result.data };
                    });
            ...
            if (parts[1] === 'mcp') {
                if (method === 'POST' && parts.length === 2) {
                    const body = await readBody();
                    return handle(async () => {
                        const result = await manager.registerMCPServer(body.server, body.agentIds || []);
                        if (!result.success)
                            return { error: result.error, status: 400 };
                        return { data: { registry: result.data, warnings: result.warnings } };
                    });
                ...
                if (method === 'PUT' && parts.length === 3) {
                    const body = await readBody();
                    return handle(async () => {
                        const result = await manager.updateMCPServer(parts[2], body.server);
                        if (!result.success)
                            return { error: result.error, status: 400 };
                        return { data: result.data };
                    });
                ...
                if (method === 'POST' && parts.length === 4 && parts[3] === 'agents') {
                    const body = await readBody();
                    return handle(async () => {
                        const result = await manager.addMCPServerToAgents(parts[2], body.agentIds || []);
                        if (!result.success)
                            return { error: result.error, status: 400 };
                        return { data: result.data };
                    });
            ...
            if (parts[1] === 'agents' && parts[2] === 'custom') {
                if (method === 'POST' && parts.length === 3) {
                    const body = await readBody();
                    return handle(async () => {
                        const result = await manager.addCustomAgent(body);
                        if (!result.success)
                            return { error: result.error, status: 400 };
                        return { data: result.data };
                    });
            ...
            if (parts[1] === 'agents' &&
                ...
                return handle(async () => {
                    ...
                    if (!result.success)
                        return { error: result.error || 'Agent not found', status: 404 };
                    return { data: result.data };
                });
            }
            ...
            if (method === 'POST' &&
                ...
                return handle(async () => {
                    ...
                    const commands = getAgentCommands(agentId, process.platform);
                    const command = action === 'install' ? commands?.install : commands?.uninstall;
                    if (!command) {
                        return {
                            error: `No automated ${action} command is catalogued for "${agentId}".${commands?.note ? ` ${commands.note}` : ' Install or uninstall it manually.'}`,
                            status: 400,
                        };
                    }
